...
std::uint16_t get_reg(std::string str) {
    ...
    if(!str.compare("M"))
        ...
        if(str.length()>2) {
            //lower
            int len = str.length();
            res = std::stoi(str.substr(str.length()-2,2), nullptr, 16) << 8;
            str = str.erase(str.length()-2,2);
            //upper
            res = res | std::stoi(str, nullptr, 16);
        } else {
            res = std::stoi(str, nullptr, 16);
        }
    }
...
std::uint32_t assemble(std::vector<std::string> ops) {
    ...
    if(!ops[0].compare("MOV"))
        ...
        tmp = ((0x06 | (params[0] << 3)) << 8) | params[1];
...
int main(int argc, char *argv[]) {
    ...
    while(ifs.good()) {
        std::getline(ifs, str);
        if(str.size())
            std::cout << std::hex << assemble(parse(str, ' ')) << std::endl;
    }
